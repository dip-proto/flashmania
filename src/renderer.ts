...
import { CarState, Track, Block, SurfaceType } from './types';
...
export class Renderer {
  ...
  private drawBlock(block: Block, checkpointsPassed: number, allCheckpoints: string[]): void {
  ...
  private drawSurfaceBlock(x: number, y: number, width: number, height: number, surface: SurfaceType): void {
  ...
  private drawBoosterBlock(x: number, y: number, width: number, height: number): void {
  ...
  private drawCheckpoint(x: number, y: number, width: number, height: number, id: string, allCheckpoints: string[]): void {
  ...
  private drawFinishLine(x: number, y: number, width: number, height: number): void {
    // Checkered finish
    const squareSize = 10;
    for (let row = 0; row < height / squareSize; row++) {
      for (let col = 0; col < width / squareSize; col++) {
  ...
  private drawStartLine(x: number, y: number, width: number, height: number): void {
    // Start line
  ...
  private drawTriggerBlock(x: number, y: number, width: number, height: number, effect: string): void {
  ...
  drawCar(car: CarState): void {
    ...
    if (isAirborne) bodyColor = '#3498db';
    if (isDrifting) bodyColor = '#f39c12';
